...
let allList = genericList.filter(item=> item.isPublished === 'true');
let cartItems=[];

$(document).ready(function() {
  showAllItems(allList);
  showCartItems(cartItems);
});

var showAllItems = function(list) {
  var source = $("#availableItems").html();
  var template = Handlebars.compile(source);
  var html = template({
    items: list
  });
  $('#availableResults').html(html);
};

var showCartItems = function(list) {
  var source = $("#cartItems").html();
  var template = Handlebars.compile(source);
  var html = template({
    items: list
  });
  $('#cartResults').html(html);
};

function action(item, type) {
  let filteredList=[];
  if(type === 'add'){
    filteredList= allList.filter(val=> val.productName !== item.productName);
    cartItems.push(item);
    allList=filteredList;
  }else{
    filteredList= cartItems.filter(val=> val.productName !== item.productName);
    allList.push(item);
